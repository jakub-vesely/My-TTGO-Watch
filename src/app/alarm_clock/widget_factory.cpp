...
#define CLICABLE_PADDING 5
#define CONTAINER_INNER_PADDING CLICABLE_PADDING * 2

lv_obj_t * wf_add_container(lv_obj_t *parent, lv_layout_t layout, lv_fit_t hor_fit, lv_fit_t ver_fit, bool add_padding){
    ...
    lv_obj_set_style_local_pad_all( container, LV_CONT_PART_MAIN , LV_STATE_DEFAULT, add_padding ? CLICABLE_PADDING : 0);
...
lv_obj_t * wf_add_tile_container(lv_obj_t *tile, lv_layout_t layout){
    lv_obj_set_style_local_pad_all(tile, LV_CONT_PART_MAIN, LV_STATE_DEFAULT, 0);
    lv_obj_t * container = wf_add_container(tile, layout, LV_FIT_PARENT, LV_FIT_TIGHT, true);
    lv_obj_align( container, tile, LV_ALIGN_IN_TOP_LEFT, 0, 0 );
    return container;
}
/**
...
lv_obj_t * wf_add_tile_footer_container(lv_obj_t *tile, lv_layout_t layout){
    lv_obj_t *container = wf_add_container(tile, layout, LV_FIT_PARENT, LV_FIT_TIGHT, true);
    lv_obj_set_style_local_pad_all( container, LV_CONT_PART_MAIN , LV_STATE_DEFAULT, CLICABLE_PADDING);
    lv_obj_set_style_local_pad_inner( container, LV_CONT_PART_MAIN , LV_STATE_DEFAULT, CONTAINER_INNER_PADDING);
    lv_obj_align( container, tile, LV_ALIGN_IN_BOTTOM_LEFT, 0, 0);//-CLICABLE_PADDING ); //-CLICABLE_PADDING is a workaround. from some reason padding is not applied and objectsa are placed to bottom of display
...
lv_obj_t * wf_add_roller(lv_obj_t *parent, char const * data, lv_roller_mode_t mode, int row_count){
    ...
    lv_obj_set_ext_click_area(roller, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING);
    return roller;
...
lv_obj_t * wf_add_switch(lv_obj_t *parent){
    ...
    lv_obj_set_ext_click_area(_switch, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING);
    return _switch;
}
...
lv_obj_t * wf_add_image_button(lv_obj_t *parent, lv_img_dsc_t const &image, lv_event_cb_t event_cb){
    ...
    lv_obj_set_ext_click_area(button, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING, CLICABLE_PADDING);
