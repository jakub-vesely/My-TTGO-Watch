...
lv_obj_t * wf_add_roller(lv_obj_t *parent, char const * data, lv_obj_t * alignment_base, lv_align_t alignment){
    ...
    // TODO: change to LV_ROLLER_MODE_INIFINITE  when will be used new TTGO_TWatch_Library where is available lvgl v7.3.1
    // currently used TTGO_TWatch_Library in version 1.2.0 with lvgl version 7.0.0(?) is not able to set roller option properly in LV_ROLLER_MODE_INIFINITE mode

    // TODO: in  the LV_ROLLER_MODE_NORMAL mode are stred correct values but it is displayed wrong value for minutes when the dialog is displayed
    // first time. by paing to any place in the dialog is value changed to the proper one
    lv_roller_set_options(roller, data, LV_ROLLER_MODE_NORMAL);
...
lv_obj_t * wf_add_labeled_switch(
    ...
    lv_obj_set_size( container, lv_disp_get_hor_res( NULL ) - offset_x * 2 , 32 );
    lv_obj_add_style( container, LV_OBJ_PART_MAIN, ws_get_container_style() );
    lv_obj_align( container, alignment_base, alignment, offset_x, offset_y);

    wf_add_label( parent, text, container, LV_ALIGN_IN_LEFT_MID, 0, 0 );
    ...
    lv_obj_align( _switch, container, LV_ALIGN_IN_RIGHT_MID, 0, 0 );
